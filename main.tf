...
resource "aws_security_group" "ms-cluster" {
  ...
  tags = {
    Name = "ms-up-running-abhi"
  }
}

resource "aws_eks_cluster" "ms-up-running-abhi" {
  name     = local.cluster_name
...
resource "aws_iam_role_policy_attachment" "ms-node-AmazonContainerRegistryReadOnly" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEC2ContainerRegistryReadOnly"
  role       = aws_iam_role.ms-node.name
}

resource "aws_eks_node_group" "ms-node-group" {
  cluster_name    = aws_eks_cluster.ms-up-running-abhi.name
  node_group_name = "microservices-abhi-learning"
  ...
  depends_on = [
    ...
    aws_iam_role_policy_attachment.ms-node-AmazonContainerRegistryReadOnly,
...
clusters:
- cluster:
    certificate-authority-data: ${aws_eks_cluster.ms-up-running-abhi.certificate_authority.0.data}
    server: ${aws_eks_cluster.ms-up-running-abhi.endpoint}
  name: ${aws_eks_cluster.ms-up-running-abhi.arn}
contexts:
- context:
    cluster: ${aws_eks_cluster.ms-up-running-abhi.arn}
    user: ${aws_eks_cluster.ms-up-running-abhi.arn}
  name: ${aws_eks_cluster.ms-up-running-abhi.arn}
current-context: ${aws_eks_cluster.ms-up-running-abhi.arn}
kind: Config
preferences: {}
users:
- name: ${aws_eks_cluster.ms-up-running-abhi.arn}
  user:
    exec:
      ...
      args:
        - "token"
        - "-i"
        - "${aws_eks_cluster.ms-up-running-abhi.name}"
    KUBECONFIG
  filename = "kubeconfig"
}